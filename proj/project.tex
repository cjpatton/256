...
Our next data set explores house value throughout California derived from 1990 census data
\footnote{See \texttt{cadata.R} and \texttt{plotmap.R} in the project folder. \texttt{Parsimony.R} has to be loaded first}. 
The state is divided up into 20640 geographic blocks with average population 1425.
...
Now we make use of our coordinate data using R's ggmap library. First we looked at a map of California as a whole, then zoomed into the Bay Area and the Greater Los Angeles area as seen in Figure~\ref{fig:housingVals}.
...
Method&Parsimony (k=0.01) & Parsimony (k=0.05) & Significance Testing \\
\hline
P($ \hat{x} == 1  | x == 1$) & 0.9862345 & 0.9861474 & 0.9864959 \\ %Correct ID $|$ class 1  % Conditional Accuracy 
\hline 											% Probability that the model is correct given that it is in class 1
P($ \hat{x} == 0  | x == 0$) & 0.9285242 & 0.9285242 & 0.9169424 \\ %Correct ID $|$ not class 1 % Conditional Accuracy (negative)
\hline
P($ x == 1  | \hat{x} == 1$) & 0.981276 & 0.9812744 & 0.9783135 \\ %Correct ID $|$ guess class 1 % True Positive
\hline
P($ x == 0  | \hat{x} == 0$) & 0.9466937 & 0.9463744 & 0.9470267  \\ %Correct ID $|$ guess not class 1 % True Negative
\hline
\end{tabular}
...
\subsection{Automobile Data Set~\cite{automobile} (continuous and logistic, large $p$, small $n$)}

% Table of Parsimonious Variables
For the small n (n \textless 1000), large p (p \textgreater 15) case, we will use the automobile data
\footnote{See \texttt{car\_analysis.R} in the project folder. \texttt{Parsimony.R} has to be loaded first}
set found on the UCI Machine Learning Repository.  This data set contains 193 instances of automobile, with 25 categories of classification for each one.
...
Implementation of \textbf{leave1out01()}
\lstinputlisting{figures/Leave_fig.R}
